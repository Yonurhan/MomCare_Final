...
# Definisikan Custom Exception untuk kejelasan
...
class MealPlanner:
    # --- PERBAIKAN KRUSIAL ---
    # Mengganti `_init_` menjadi `__init__` agar konstruktor dipanggil dengan benar.
    ...
    def _get_filtered_foods(self, nutrient: str) -> List[Dict]:
        ...
        for rec in all_recs:
            ...
            # 2. Proses item makanan dengan memeriksa keberadaan kunci 'food'
            if 'food' in rec:
    ...
    def generate_plan(self, deficient_nutrients: List[str]) -> Optional[Dict]:
        ...
        for nutrient in deficient_nutrients:
            # Dapatkan hanya item makanan dari rekomendasi
        ...
        # Urutkan makanan berdasarkan seberapa banyak nutrisi defisit yang mereka cakup
        ...
        # Cari opsi sarapan dari makanan yang belum digunakan
...
class WeeklyAssessmentService:
    ...
    def _load_context_and_init_planner(self):
        ...
        # Inisialisasi MealPlanner dengan data yang benar
    ...
    def _calculate_targets(self):
        # Gunakan nilai default jika atribut user tidak ada
        weight = self.user.weight or 60
        height = self.user.height or 160
        age = self.user.age or 29

        dynamic_targets = calculate_nutrition_goals(age=age, weight=weight, height=height, lmp_date=self.user.lmp_date)
    ...
    def _process_logs(self):
        ...
        # Agregasi log per tanggal untuk menghindari duplikasi jika ada beberapa log di hari yang sama
        logs_by_date = {}
        ...
        # Hitung total mingguan dari data yang sudah diagregasi
    ...
    def _analyze_risks(self):
        for nutrient, days in self.metrics.get('days_completed', {}).items():
            # Analisis hanya jika hari pemenuhan kurang dari ambang batas (misal, 5 hari)
            if days < 5:
    ...
    def _score_and_create_alert_for_nutrient(self, nutrient: str, days_completed: int):
        ...
        # Skor dasar berdasarkan konsistensi
        score = (1 - (days_completed / 7)) * 40
        
        # Cek data historis untuk isu berulang
        ...
        # Tambahkan bobot jika isu berulang
        if recurrence_count >= 2: score *= 1.5
        elif recurrence_count == 1: score *= 1.2
        
        # Tambahkan bobot untuk kondisi medis yang sudah ada
        ...
        # Jangan buat alert jika skor terlalu rendah
        if score < 15: return
        ...
        # Panggil MealPlanner untuk mendapatkan rekomendasi makanan yang sudah difilter
    ...
    def _score_and_create_alert_for_symptom(self, symptom: str):
        ...
        # Tambahkan bobot jika gejala terkait dengan kekurangan nutrisi yang terdeteksi
    ...
    def _generate_weekly_goals(self) -> List[Goal]:
        ...
        # Ambil 2 alert dengan skor tertinggi
        goals = []
        ...
        for i, alert in enumerate(top_alerts):
            ...
            if alert.category == 'nutrition':
                ...
                description = "Pilih dan terapkan 2 tips gaya hidup yang kami sarankan untuk mengurangi gejala ini."
    ...
    def _compile_final_results(self, goals: List[Goal]) -> Dict:
        # Tentukan nutrisi yang kurang berdasarkan alert yang signifikan
